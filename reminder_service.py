...
# 創建FastAPI應用
app = FastAPI(title="客戶二次測試提醒系統API")
...
def send_email(to_email, subject, html_content):
    ...
    try:
        ...
        # 這裡應該實現實際的郵件發送邏輯
        # 由於這是一個模擬，我們只返回成功
        print(f"模擬發送郵件到 {to_email}，主題：{subject}")
        return True
    except Exception as e:
        print(f"發送郵件時出錯: {e}")
        return False
...
@app.post("/api/reminders/settings", response_model=ReminderSettings)
...
@app.post("/api/reminders/check", response_model=Dict[str, Any])
async def check_reminders(background_tasks: BackgroundTasks, _: str = Depends(get_current_admin)):
    """檢查並發送提醒"""
    return check_and_send_reminders(background_tasks)

@app.post("/api/reminders/test", response_model=Dict[str, Any])
async def test_reminder_email(data: Dict[str, str] = Body(...), _: str = Depends(get_current_admin)):
    """測試提醒郵件"""
    try:
        ...
        test_reminder = {
            ...
            "health_data": {
                ...
                "symptoms": ["失眠", "疲勞", "頭痛"],
                "bodySystemIssues": ["神經系統", "消化系統"],
                "specificConditions": ["乾眼症"]
            },
            "recommendations": {
                "supplements": ["魚油", "B群", "維他命C", "鈣鎂鋅"]
            }
        }
        
        # 生成提醒郵件內容
        html_content = generate_reminder_email(test_reminder)
        ...
        if success:
            return {"success": True, "message": f"測試郵件已發送到 {email}"}
        else:
            return {"success": False, "message": "發送測試郵件失敗，請檢查郵件設置"}
    except HTTPException:
        raise
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"發送測試郵件時出錯: {str(e)}")

@app.post("/api/reminders/create", response_model=Dict[str, Any])
async def create_reminder_api(data: Dict[str, str] = Body(...), _: str = Depends(get_current_admin)):
    """手動創建提醒"""
    try:
        email = data.get("email")
        ...
        # 創建提醒
        create_reminder(email, report_id)
        
        return {"success": True, "message": f"已為 {email} 創建提醒"}
    except HTTPException:
        raise
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"創建提醒時出錯: {str(e)}")

@app.delete("/api/reminders/{reminder_id}", response_model=Dict[str, Any])
async def delete_reminder(reminder_id: str, _: str = Depends(get_current_admin)):
    """刪除提醒"""
    try:
        result = reminders_collection.delete_one({"_id": reminder_id})
        
        if result.deleted_count == 0:
            raise HTTPException(status_code=404, detail=f"找不到提醒: {reminder_id}")
        
        return {"success": True, "message": f"提醒已刪除: {reminder_id}"}
    except HTTPException:
        raise
    except Exception as e:
        print(f"刪除提醒時出錯: {e}")
        ...
        if 'reminders_db' in globals():
            for i, r in enumerate(reminders_db):
                if str(r.get("_id")) == reminder_id:
                    del reminders_db[i]
                    return {"success": True, "message": f"提醒已刪除: {reminder_id}"}
            raise HTTPException(status_code=404, detail=f"找不到提醒: {reminder_id}")
        raise HTTPException(status_code=500, detail=f"刪除提醒時出錯: {str(e)}")
